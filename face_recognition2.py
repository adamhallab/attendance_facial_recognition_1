...
server.login("Enter email username here", "Enter email password here")
server.sendmail(
    "From email",
    "To email",
    "\n{} Has Checked In at {} on {}".format(myName, current_time, date))
server.quit()
...
db.child("users").child("Timesin").set(times)
db.child("users").child("Timesout").set("-")
db.child("users").child("Dates").set(dates)
db.child("users").child("Status").set("At Work")
